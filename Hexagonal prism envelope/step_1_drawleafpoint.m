...
%   leaf = load(['H:\Dongni Li\Leaf\leaf(' num2str(a) ').txt']);
    ...
    plot3(leaf(:,1),leaf(:,2),leaf(:,3),'.','Markersize',8,'Color',[r1,r2,r3]);
    hold on
    lab = [r1 r1 r3];
%   lab = [rand rand rand];
    leaves{a,1}=leaf(:,1:3);
...
zhi = load('D:\matlab Project\Dongni Li UPDATE\叶子\10.23图9右边的树\图9右边leaf2 - branch.txt');
plot3(zhi(:,1),zhi(:,2),zhi(:,3),'k.','markersize',4);
