...
% end
% 画叶子投影点位信息
plot3(x(:,1),x(:,2),x(:,3),'.','color','g','Markersize',1);
hold on;
...
%画下面儿ABCDEF
fill3([box(1:6,1);box(1,1)],[box(1:6,2);box(1,2)],[box(1:6,3);box(1,3)],color,'FaceAlpha',0.05,'LineWidth',1);
hold on;
%画上面儿A'B'C'D'E'F'
% 上面小
...
fill3([box(7:12,1);box(7,1)],[box(7:12,2);box(7,2)],[box(7:12,3);box(7,3)],color,'FaceAlpha',0.05,'LineWidth',1);
hold on;
...
%把ABCDE对应的A'B'C'D'E'F'之间的连线画好，就形成了一个盒子
    for k=1:5
    fill3([box(k,1);box(k+6,1);box(k+7,1);box(k+1,1)],[box(k,2);box(k+6,2);box(k+7,2);box(k+1,2)],[box(k,3);box(k+6,3);box(k+7,3);box(k+1,3)],color,'FaceAlpha',0.05,'LineWidth',1);
    hold on;
...
zhi = load('D:\matlab Project\Dongni Li UPDATE\叶子\10.22补充枝干\图9branch5.txt');
%plot3(zhi(:,1),zhi(:,2),zhi(:,3),'k.','markersize',3);
