clear;
clc;
path = 'D:\matlab Project\Dongni Li UPDATE\Leaf\';
files = dir(strcat(path,'*.txt'));
for leafid = 1:size(files,1)
    leafid
    leaf = load(['D:\matlab Project\Dongni Li UPDATE\Leaf\leaf(' num2str(leafid) ').txt']);
...
%     end
    ...
    for i = 1:max(flag)
        ...
        % 保存
        filename=['D:\matlab Project\Dongni Li UPDATE\leaf_cutAgain\leaf(' num2str(leafid) ')_' num2str(cutid) '.txt'];
